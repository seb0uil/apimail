...
public class ConfigurableNettyJaxrsServer extends NettyJaxrsServer {
    ...
    public Bootstrap initBootstrap() {
        this.bootstrap = new ServerBootstrap(
                new NioServerSocketChannelFactory(Executors.newCachedThreadPool(), Executors.newCachedThreadPool(), ioWorkerCount));
        return bootstrap;
    }
    ...
    public void start() {
        ...
        List<ChannelHandler> additionalChannelHandlers = new ArrayList<ChannelHandler>();
        if (sslContext == null) {
            factory = new HttpServerPipelineFactory(dispatcher, root, executorThreadCount, maxRequestSize, false, additionalChannelHandlers);
        } else {
            factory = new HttpsServerPipelineFactory(dispatcher, root, executorThreadCount, maxRequestSize, false, additionalChannelHandlers, sslContext);
        }
